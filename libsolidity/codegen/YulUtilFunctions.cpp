	...
	return m_functionCollector.createFunction(functionName, [&]() {
		...
		return Whiskers(R"(
			function <functionName>(condition <messageVars>) {
				if iszero(condition) {
					let mem := <allocateUnbounded>()
					mstore(mem, <errorHash>)
					let end := <abiEncodeFunc>(add(mem, <hashHeaderSize>) <messageVars>)
					revert(mem, sub(end, mem))
				}
			}
		)")
		("functionName", functionName)
		...
		if (_from.baseType()->isValueType())
		{
			...
			return Whiskers(R"(
				function <functionName>(slot) -> memptr {
					...
					<finalize>(memptr, sub(end, memptr))
				}
			)")
			("functionName", functionName)
			("allocateTemp", allocateUnboundedFunction())
			(
				"encode",
				abi.abiEncodeAndReturnUpdatedPosFunction(_from, _to, ABIFunctions::EncodingOptions{})
			)
			("finalize", finalizeAllocationFunction())
			.render();
		}
		else
		{
	...
	return m_functionCollector.createFunction(functionName, [&]() {
		return util::Whiskers(R"(
			function <functionName>() -> ret {
				...
				<finalize>(data, add(add(offset, msg), add(0x20, length)))
				ret := msg
			}
		)")
		("functionName", functionName)
